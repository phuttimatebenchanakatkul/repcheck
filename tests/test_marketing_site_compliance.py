...
* THE HANDSET NEVER TAKES FOCUS. The food log's walkthrough called .focus()
  to keep the keyboard on whichever sheet it had just opened, and it opens
  them on a loop -- so a reader scrolling past watched activeElement travel
  from BODY into the mock app on its own, which also took Page Down, Home and
  End away from them. The demos are decorative; they drive their look from
  classes and attributes, and nothing in the handset is in the tab order.
...
def test_the_focus_visible_rules_actually_draw_something():
    ...
    against --app-line, so on the phone screens it is invisible.

    The handset is tabindex="-1" throughout now (see the section below), but
    that is not the same as unfocusable -- clicking into the food log's
    search or amount field still focuses it, and a text field matches
    :focus-visible on a mouse click -- so the .phone-screen override still
    has work to do.
    """
    css = _strip_css_comments((MARKETING / "styles.css").read_text(encoding="utf-8"))
...
# ---------- The handset never takes focus ----------

def _handset_markup():
    """index.html's phone stage: the handset, its six screens and the watch
    rig beside it. Everything in here is a picture of the app."""
    html = _read("index.html")
    start = html.index('<div class="phone-stage">')
    return _strip_comments(html[start:html.index("</section>", start)])


FOCUSABLE_TAG = r"<(?:a|button|input|select|textarea)\b[^>]*>"


def test_nothing_in_the_handset_is_in_the_tab_order():
    """The phone is a drawing of the app, not a form the page is offering. A
    reader tabbing this page should reach the nav, the waitlist and the
    footer -- not nine mock app controls in between.

    Most of the panes make the point by not using focusable elements at all:
    the challenges and friends screens are <div>/<span> throughout. The food
    log cannot -- it really computes, so its controls are real controls --
    and says it with tabindex instead.
    """
    offenders = [
        tag for tag in re.findall(FOCUSABLE_TAG, _handset_markup())
        if 'tabindex="-1"' not in tag
    ]

    assert offenders == [], (
        "these controls inside the handset are still tab-reachable: "
        + repr(offenders)
        + " -- give them tabindex=-1, or render them as the <div>/<span> the "
        "challenges and friends screens use"
    )


def test_the_handset_controls_app_js_renders_are_out_of_the_tab_order_too():
    """Half the handset's controls are written by app.js rather than by
    index.html -- the HYROX station chips, the race CTAs, the food log's
    search results, the watch rig's button. Pinning only the markup would
    leave every one of those tab-reachable, and they are the ones easiest to
    forget: they are string concatenation, not something you see while
    reading the page.

    Checked per string literal rather than per tag, because the tag and its
    attributes are split across a concatenation and only the opening literal
    carries them: '<button type="button" tabindex="-1" class="stchip" ...'.
    The literal pattern honours backslash escapes. It has to: app.js really
    does write `'<span class="sub">Men\\'s Open Singles</span>'` a few lines
    from the HYROX chips. A pattern that stopped at the first quote would
    split that literal in two and re-sync on the wrong side of it, so whether
    a control's `tabindex` landed in the reported half would be luck.
    """
    js = (MARKETING / "app.js").read_text(encoding="utf-8")
    literals = re.findall(r"'(?:[^'\\\n]|\\.)*'|\"(?:[^\"\\\n]|\\.)*\"", js)
    opens_a_control = [
        lit for lit in literals
        if re.search(r"<(?:a|button|input|select|textarea)\b", lit)
    ]

    # Without this the assertion below passes on an empty list -- which is
    # exactly what a change to how these rows are built would produce.
    assert len(opens_a_control) >= 12, (
        "found only " + str(len(opens_a_control)) + " generated controls in "
        "app.js; the markup moved and this test is now checking nothing"
    )

    offenders = [lit for lit in opens_a_control if 'tabindex="-1"' not in lit]
    assert offenders == [], (
        "app.js renders these into the handset without tabindex=-1: "
        + repr(offenders)
    )


def test_the_handset_walkthroughs_never_call_focus():
    """THE REGRESSION THIS SECTION EXISTS FOR.

    The food log's sheets used to call .focus() so the keyboard followed
    whichever one was open -- right for a real dialog, wrong here, because
    the walkthrough opens and closes those same sheets by itself on a
    nine-second loop. On the live site document.activeElement travelled
    BODY -> BUTTON.cta-blue -> INPUT.nl-input while the reader was doing
    nothing but scrolling past. Two faults in one:

      * focus moved somewhere nobody put it and nobody could predict; and
      * a focused text input changes what Page Down, Page Up, Home and End
        do -- and on the canvas host those keys are ours, forwarded to the
        body scroller by a handler that stands down the moment anything is
        focused. The loop was taking the page keys away mid-scroll.

    The demos drive their visuals from classes and attributes instead:
    data-wl for the workout log, .is-faux-focus for the food log's caret.
    The only .focus() left on the site is the waitlist form pointing at an
    address it just rejected, which is a real control answering a real
    submit.
    """
    js = (MARKETING / "app.js").read_text(encoding="utf-8")
    # Comments only, so the prose above ("used to call .focus()") cannot fail
    # its own assertion. Line comments are the only kind app.js uses here.
    js = re.sub(r"//[^\n]*", "", js)

    waitlist = js.index("var ENDPOINT")
    handset, form = js[:waitlist], js[waitlist:]

    assert ".focus(" not in handset, (
        "something above the waitlist block calls .focus(). Everything up "
        "there drives the handset mockups, and a mockup that moves focus "
        "moves it for a reader who is only scrolling past. Render the "
        "focused look with a class -- see nlFakeFocus -- instead."
    )
    assert form.count(".focus(") == 1, (
        "the waitlist's focus-the-rejected-address call changed shape. "
        "Without it a screen-reader user who submits a bad address is told "
        "so with no way back to the field."
    )


def test_the_food_log_caret_is_drawn_for_the_class_as_well_as_for_focus():
    """A class nothing styles is a walkthrough whose search field never
    lights up -- which looks broken rather than wrong, and is the harder of
    the two to notice. Both fields carry the look on .is-faux-focus AND on a
    real :focus, because a visitor can still click into them.
    """
    css = _strip_css_comments((MARKETING / "styles.css").read_text(encoding="utf-8"))
    rules = re.findall(r"([^{}]+)\{([^}]*)\}", css)

    for field in (".nl-input", "#nl-amount"):
        faux = [body for sel, body in rules if field in sel and ".is-faux-focus" in sel]
        assert faux, (
            field + " has no .is-faux-focus rule -- the walkthrough's caret "
            "would draw nothing at all"
        )
        assert any("border-color" in body for body in faux), (
            field + "'s .is-faux-focus rule sets no border-color: " + repr(faux)
        )
        assert any(field + ":focus" in sel for sel, _ in rules), (
            field + " lost its :focus rule -- tabindex=-1 keeps it out of the "
            "tab order but a click still focuses it"
        )
