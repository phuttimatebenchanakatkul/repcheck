...
(function () {
  ...
  // Same illustration as coaching.js's bodyTypeIconSvg, copied verbatim
  // (duplicated rather than shared -- see file header) -- an earlier,
  // trimmed-down duplicate here dropped the hairstyle/garment shapes that
  // actually make "male" vs "female" legible at a glance, leaving only a
  // subtle waist-taper difference that read as ambiguous (reported as
  // "picked male, body type step still looked female"). This is the full
  // version: skin-toned body + a separate garment overlay (shirt+pants vs
  // top+skirt) + hair silhouette (short cap vs long flowing), not just a
  // proportion tweak.
  function bodyTypeIconSvg(gender, index, total) {
    ...
    return `
      <svg width="60" height="86" viewBox="0 0 60 96" stroke="none">
    ...
    ranges.forEach((r, i) => {
      ...
      grid.appendChild(el(`
        <button type="button" class="ob-body-type-card ${isSelected ? "is-selected" : ""}" data-action="set-body-type" data-value="${r.id}">
          ...
          <div class="ob-body-type-icon">${bodyTypeIconSvg(gender, i, ranges.length)}</div>
